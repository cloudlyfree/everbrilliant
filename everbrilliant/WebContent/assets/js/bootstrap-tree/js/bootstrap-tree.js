 ...
 * ============================================================ */
var bootstrap_currentNode = null;
!function ($) {

  "use strict"; // jshint ;_;

  var loading = "<img src='/assets/js/bootstrap-tree/img/ajax-loader.gif' class='indicator' /> Loading ...";

  /* TREE CLASS DEFINITION
   * ========================= */
  
  var Tree = function (element, options, event) {
    ...
    this.options.toggle && this.toggle(event)
  }
  ...
  Tree.prototype = {
    ...
    , toggle: function (e) {
      ...
      if(bootstrap_currentNode){
        bootstrap_currentNode.removeClass('currentNode');
      }
      bootstrap_currentNode = this.$parent;
      bootstrap_currentNode.addClass('currentNode');

      if(srcElement && (srcElement.tagName=="SPAN") && this.$element.hasClass("in")){
        return;
      }
    ...
    , _createNodes: function (nodes) {
      ...
      $.each(nodes, function (ind, el) {
        ...
        if (!el.leaf) {
          ...
          attributes.className = "tree-toggle closed"
  ...
  $.fn.tree = function (option, event) {
    ...
    return this.each(function () {
      ...
      if (!data) $this.data("tree", (data = new Tree(this, options, event)))
      if (typeof option == "string") data[option](event)
    })
  ...
  $(function () {
    ...
    $("body").on("click.tree.data-api", "[data-toggle=branch]", function (e) {
      ...
      $(target).tree(option, e)
     // _alert($("#treeDiv").offset().top - $this.offset().top)
      $("#treebgbanner").css("top",$this.offset().top - $("#treeDiv").offset().top - 3 + "px");
      //return false
    })
