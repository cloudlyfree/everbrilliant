...
!function ($) {
	...
	function Multiselect(select, options) {
		...
		if(options.fixpos)
		{
			$('button', this.$container).parent().css({"position":"static"});
			if (this.options.maxHeight) {
				$('ul', this.$container).css({
					'position' : 'absolute',
					'left' : 'auto',
	...
	Multiselect.prototype = {
		...
		defaults: {
			...
			// If more than 1 options are selected, the number of selected options is printed.
			buttonText: function(options) {
				if (options.length == 0) {
					return '尚未选择 <b class="caret"></b>';
				}
				else if (options.length > 1) {
					return options.length + ' 项选中 <b class="caret"></b>';
				}
				else {
					var selected = '';
