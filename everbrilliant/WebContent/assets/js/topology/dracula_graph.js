...
Graph.Renderer.Raphael = function(element, graph, width, height) {
    ...
    if(window.matchMedia)
        matchmedia = window.matchMedia("(max-width: 524px)").matches;
    else
        matchmedia = false;
    this.width = width || 400;
    if(matchmedia) this.width = 400;
            ...
            node.render = function(r, node) {
                ...
                if(node.src){   
                   ...
                   var browsers = navigator.userAgent;
                    if(browsers.indexOf("MSIE 6.0") < 0 && browsers.indexOf("MSIE 7.0") < 0 && browsers.indexOf("MSIE 8.0") < 0)
                       element = r.image(node.src, 0, 0, node.height, node.width).attr({fill: color, stroke: color, "stroke-width": 0,"title":node.label});
                    else if(browsers.indexOf("MSIE 7.0") > 0)
                       element = r.rect(0, 0, node.height, node.width).attr({fill: "url("+node.src+")", stroke: color, "stroke-width": 0, "stroke-opacity":1,"title":node.label});
                    else 
                       element = r.rect(0, 0, node.height, node.width).attr({fill: "url("+node.src+")", stroke: color, "stroke-width": 0, "stroke-opacity":1,"title":node.label});
                }
                else 
                    element = r.ellipse(0, 0, 30, 20).attr({fill: color, stroke: color, "stroke-width": 2});
                if(node.onclick)
                    element.mousedown(node.onclick);
                var nameStr = node.label.split("\n");
               // alert(nameStr[0] + nameStr[1] +nameStr[2])
                if(nameStr[0].length > 7)
                    nameStr[0] = nameStr[0].substring(0, 7) + "...";
                node.label = nameStr.join("\n");
                /* set DOM node ID */
                element.node.id = node.label || node.id;
                element.data = node;
                shape = r.set().
                    push(element).
                    push(r.text(node.src ? node.width/2 : 0, node.src ? node.height+10 : 30, node.label || node.id));
                return shape;
            }
        }
...
Graph.Layout.Tree.prototype = {
    ...
    */
    layoutHorizontally: function() {
        ...
        for (i in this.graph.nodelist) {
            ...
            node.layoutPosX += distanceToRoot;
        }
    },
    ...
    */
    layoutVertically: function() {
        ...
        for (i = 0; i <= this.depth; i++) {
            ...
            for (j in this.levels[i]) {
                ...
                //唯一子节点layoutPosY等于父节点的layoutPosY
                if(
                    node.edges && (edge = node.edges[0].backedge)
                    &&
                    (
                        (len = (source = edge.source).edges.length) === 1
                        ||
                        (len === 2 && source.edges[0].backedge)
                    )
                ) node.layoutPosY = source.layoutPosY;
            }
        }
    },
    ...
    getTargetNodes: function(node) {
        var nodes = [];
        for (var i = 0; i < this.graph.edges.length; i++) {
            if (this.graph.edges[i].target == node && !this.graph.edges[i].backedge) {
