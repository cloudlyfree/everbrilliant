    ...
    DateRangePicker.prototype = {
        ...
        show: function (e) {
            ...
            $(document).on('mousedown', function() {
                /*[增*/ 
                $("body").attr("noChange", "true");
                $(".daterangepicker").hide();
                /*增]*/

                /*减 $.proxy(this.hide, this) 减*/
            });
            
            /*[增*/   
            if($(".daterangepicker .end-date").next("td").length===0){
                $(".daterangepicker .end-date").parent().next().find("td:first").trigger("click");
            }else{
                $(".daterangepicker .end-date").next("td").trigger("click");
            };
            if($(".daterangepicker .end-date").prev("td").length===0){
                $(".daterangepicker .end-date").parent().prev().find("td:last").trigger("click");
            }else{
                $(".daterangepicker .end-date").prev("td").trigger("click");
            };
            /*增]*/
        },
        ...
        hide: function (e) {
            ...
            if (this.changed) {
                ...
                /*[增*/
                if ($("body").attr("noChange")==="true") return false;
                /*增]*/
        ...
        enterDate: function (e) {
            ...
            var _title = $(e.target).attr('_title');
            var row = _title.substr(1, 1);
            var col = _title.substr(3, 1);
            var cal = $(e.target).parents('.calendar');
        ...
        updateCalendars: function () {
            ...
            for (var range in this.ranges) {
                if (this.startDate.equals(this.ranges[range][0]) && this.endDate.equals(this.ranges[range][1])) {
                    customRange = false;
                    if (typeof cb == 'function' && !this.isClick){
                    	cb(this.startDate,this.endDate);
                    }
                    this.isClick = false;
        ...
        renderCalendar: function (calendar, selected, minDate, maxDate) {
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    var _title = 'r' + row + 'c' + col;
                    html += '<td class="' + cname.replace(/\s+/g,' ').replace(/^\s?(.*?)\s?$/,'$1') + '" _title="' + _title + '">' + calendar[row][col].getDate() + '</td>';
                }
                html += '</tr>';
            }
