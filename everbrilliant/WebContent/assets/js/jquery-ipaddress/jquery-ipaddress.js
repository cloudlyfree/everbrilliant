(function($){
	$.fn.ipaddress = function() {
		return this.each(function(){
			...
			if($this.next('.ip_container')){
				$this.removeClass('ip-enabled');
				$this.next('.ip_container').remove();
			}
			if (!$this.hasClass('ip-enabled')) {
				...
				var octets = [];
				var id_prefix = $this.attr('id').replace(/\[/g, '_').replace(/\]/g, '');
				for (var i = 0; i <=3; i++) {
					octets.push('<input class="ip_octet" id="' + id_prefix + '_octet_' + (i+1) + '" maxlength="3" value="' + ip_value[i] + '" />');
				}
				var octet_html = octets.join('.');
				$this.after($('<div class="ip_container" style="display: inline-block;*display:inline;"/>').html(octet_html));
			...
			$('input.ip_octet', $this.next('.ip_container')).bind('keydown', function(e){
				...
				//if ($('input.ip_octet', $(this).parent()).filter(function(){ return this.value.length; }).length == 4) {
					...
					$('input.ip_octet', $(this).parent()).each(function(){
						ip_value.push(this.value>255?255:this.value);
					});
					var address_value = ip_value.join('.');
					if(address_value=="...")
						address_value = "";
					$this.val(address_value);
				//}
			}).bind('blur', function(){
				if (this.value > 255) this.value = 255;
			});

            $this[0].setAttribute('data-ipinput', 'true');
            $this.next()[0].setAttribute('data-ipcontainer', 'true');
    ...
    $.fn.addressReadOnly = function(disabled){

        var fn = disabled ? function($e){
            $e.addClass('fix-ipContainerDisabled')
            $e.children().attr({readonly: 'readonly'});
        } : function($e){
            $e.removeClass('fix-ipContainerDisabled');
            $e.children().removeAttr('readonly');
        };

        this.each(function(){
            var $inputContainer;
            if(this.getAttribute('data-ipinput') &&
                ($inputContainer = $(this).next())[0].getAttribute('data-ipcontainer') &&
                    fn($inputContainer));
        });
